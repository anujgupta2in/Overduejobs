...
def analyze_overdue_jobs(df):
    ...
    if 'Calculated Due Date' not in df.columns or 'Job Status' not in df.columns:
        return results
    
    # Make a copy to avoid modifying the original
    df_copy = df.copy()
    
    # Convert due date to datetime (handle various date formats)
    ...
    # Initialize counts
    total_jobs = len(df_copy)
    overdue_jobs_all = pd.DataFrame()
    critical_overdue_jobs_all = pd.DataFrame()
    
    # Check if there's a file identifier column to process per file
    if '_source_file' in df_copy.columns or 'File Name' in df_copy.columns:
        # Determine which column to use as file identifier
        ...
        for file_name in files:
            ...
            file_total_jobs = len(file_data)
            overdue_percentage = round((overdue_jobs_count / file_total_jobs) * 100, 1) if file_total_jobs > 0 else 0
            critical_percentage = round((critical_overdue_jobs_count / file_total_jobs) * 100, 1) if file_total_jobs > 0 else 0
            
            # Add file results to the list
            results['file_results'].append({
                'file_name': file_name,
                'total_jobs': file_total_jobs,
                'overdue_jobs_count': overdue_jobs_count,
                'overdue_jobs_percentage': overdue_percentage,
                'critical_overdue_jobs_count': critical_overdue_jobs_count,
                'critical_overdue_jobs_percentage': critical_percentage
            })
            
            # Collect all overdue jobs for overall statistics
            overdue_jobs_all = pd.concat([overdue_jobs_all, overdue_jobs])
            critical_overdue_jobs_all = pd.concat([critical_overdue_jobs_all, critical_overdue_jobs])
        
        # Calculate overall statistics
        results['total_jobs'] = total_jobs
        results['overdue_jobs_count'] = len(overdue_jobs_all)
        results['critical_overdue_jobs_count'] = len(critical_overdue_jobs_all)
        
        # Calculate overall percentages
        results['overdue_jobs_percentage'] = round((results['overdue_jobs_count'] / total_jobs) * 100, 1) if total_jobs > 0 else 0
        results['critical_overdue_jobs_percentage'] = round((results['critical_overdue_jobs_count'] / total_jobs) * 100, 1) if total_jobs > 0 else 0
        
        # Store the actual overdue job dataframes
        results['overdue_jobs'] = overdue_jobs_all
        results['critical_overdue_jobs'] = critical_overdue_jobs_all
...
def create_overdue_jobs_chart(overdue_data, critical_data):
    ...
    categories = ['Overdue Jobs', 'Critical Overdue Jobs']
